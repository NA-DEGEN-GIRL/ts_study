...
// 연습 1: 적절한 타입을 사용하세요
// 풀이: TypeScript는 타입 추론 (Type Inference)을 하지만, 명시적으로 타입을 지정할 수도 있습니다
// 명시적 타입 지정은 코드의 의도를 명확히 하고, 예상치 못한 타입 변경을 방지합니다
...
// 풀이: Enum은 관련된 상수들을 그룹화합니다. 기본적으로 0부터 시작하는 숫자가 할당됩니다
// Enum을 사용하면 의미 있는 이름으로 상수를 관리할 수 있어 코드 가독성이 향상됩니다
enum OrderStatus {
  PENDING,     // 0
  PROCESSING,  // 1
  SHIPPED,     // 2
...
// 풀이: Enum 멤버를 값으로 사용할 수 있습니다
const currentStatus: OrderStatus = OrderStatus.PROCESSING;

// 연습 3: 타입 단언 (Type Assertion)을 사용하세요
// 풀이: as 키워드로 타입을 단언합니다. 개발자가 타입을 더 잘 알고 있을 때 사용합니다
// TypeScript 컴파일러보다 개발자가 타입 정보를 더 정확히 알고 있는 경우에 유용합니다
...
// 연습 4: any vs unknown
// 풀이: unknown은 any와 달리 타입 체크를 강제합니다. 더 안전한 타입입니다
// any는 모든 타입 체크를 우회하지만, unknown은 사용 전에 타입을 확인해야 합니다
function processValue(value: unknown): string {
  // typeof를 사용하여 타입 가드 (Type Guard)를 적용합니다
...
// 풀이: 튜플은 고정된 길이와 각 위치마다 다른 타입을 가질 수 있는 배열입니다
// 배열과 달리 각 요소의 타입과 개수가 정해져 있어 더 엄격한 타입 체크가 가능합니다
